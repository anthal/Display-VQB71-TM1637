...
# WLAN-Konfiguration
wlanSSID = settings.net['ssid']
wlanPW = settings.net['key']
network.country(settings.net['country'])

# MQTT-Konfiguration
mqttBroker = settings.mqtt['mqttBroker']
mqttClient = settings.mqtt['mqttClient']
mqttUser = settings.mqtt['mqttUser']
mqttPW = settings.mqtt['mqttPW']
mqttTopic = settings.mqtt['mqttTopic']

# Winterzeit / Sommerzeit
GMT_OFFSET = 3600 * int(settings.time['gmt_offset'])

# Status-LED
led_onboard = machine.Pin('LED', machine.Pin.OUT, value=0)

# NTP-Host
NTP_HOST = settings.time['ntp_server']
...
def mqttConnect():
    if mqttUser != '' and mqttPW != '':
        print("MQTT-Verbindung herstellen: Client '%s' zu '%s' als '%s'" % (mqttClient, mqttBroker, mqttUser))
