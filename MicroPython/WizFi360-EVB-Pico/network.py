...
class WLAN():
    ...
    # internal Functions:
    def sendCMD_waitResp(self, cmd, timeout=3000, debug=True):
        if debug:
            print("CMD: " + cmd)
    ...
    def sendATcmd_waitResp(self, cmd, timeout=3000, debug=True):
        if debug:
            print("CMD: AT+" + cmd)
        uart.write("AT+{}\r\n".format(cmd))
        resp = self.waitResp(timeout)
    ...
    def waitResp(self, timeout=3000):
        stamp = utime.ticks_ms()/1000
        response = b""
        while (utime.ticks_ms()/1000 - stamp) < timeout:
            stamp = utime.ticks_ms()/1000
            if uart.any(): 
    ...
    def tcp_close(self, linkID):
        ...
        self.send_cmd('AT+CIPCLOSE={}'.format(linkID), 'OK')
    ...
    def send_cmd(self, cmd, ack, timeout=2000):
        ...
        uart.write('{}\r\n'.format(cmd))
        retval, s = self.wait_ack(ack, timeout)
        return retval, s
